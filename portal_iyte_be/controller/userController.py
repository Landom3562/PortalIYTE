...
import requests
from config import sql_address, backend_endpoint, frontend_endpoint, MAIL_USERNAME, MAIL_PASSWORD, MAIL_DEFAULT_SENDER, sql_user, sql_password, sql_database
...
def set_bio(conn):
    with conn.cursor() as cur:
        ...
        try:
            ...
            return "Bio successfully set.", 200
        except mysql.connector.Error as e:
            return str(e), 500
...
def set_profile_picture(conn):
    with conn.cursor() as cur:
        ...
        try:
            ...
            return "Profile picture successfully set.", 200
        except mysql.connector.Error as e:
            return str(e), 500
...
def get_comments(conn, user_id):
    with conn.cursor(dictionary=True) as cur:
        cur.execute("SELECT c.id as id, user_id, post_id, parent_id, has_parent, like_count, comment_count, content, c.create_date as create_date, username FROM comment c, user u WHERE c.user_id = %s AND c.user_id = u.id", [user_id])
...
@user_bp.route(prefix + '/report', methods=["POST"])
@ensure_connection
def report_user(conn):
    with conn.cursor() as cur:
        body = request.json
        try:
            cur.execute("INSERT INTO report (reporter_id, reported_id, report_title, reason) VALUES (%(reporterId)s, %(reportedId)s, %(reportIssue)s, %(reason)s)", body)
            return "200", 200
        except mysql.connector.Error as e:
            return str(e), 500
